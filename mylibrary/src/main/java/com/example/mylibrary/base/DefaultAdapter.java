...
public abstract class DefaultAdapter<T> extends RecyclerView.Adapter<BaseHolder<T>> {
    ...
    public BaseHolder<T> onCreateViewHolder(ViewGroup parent, final int viewType) {
        ...
        mHolder.setOnItemClickListener(new BaseHolder.OnViewClickListener() {
            ...
            public void onViewClick(View view, int position) {
                if (mOnItemClickListener != null && mLists.size() > 0) {
                    mOnItemClickListener.onItemClick(view, viewType, mLists.get(position), position);
                }
            }
        });
        return mHolder;
    ...
    public void onBindViewHolder(BaseHolder<T> holder, int position) {
        holder.setData(mLists.get(position), position);
    }
